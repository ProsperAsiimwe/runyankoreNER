...
def extract_config_info(path):
    model_match = re.search(r"combined/(.*?)/", path)
...
def compile_ranked_similarity(base_dir="MEXA_inspired_strategy/outputs/combined"):
    all_rows = []
...
def generate_model_tables(df, model, output_dir="combined_latex"):
...
def generate_visualizations(df, output_dir="combined_latex"):
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Heatmap: Similarity by Language and Config
...
if __name__ == "__main__":
    os.makedirs("combined_latex", exist_ok=True)
    df = compile_ranked_similarity()
    df.to_csv("combined_latex/compiled_similarity.csv", index=False)

    for model in ["xlmr", "mbert"]:
        generate_model_tables(df, model)

    generate_visualizations(df)
    print("[SUCCESS] Clean LaTeX tables and PNG charts saved to: combined_latex/")
