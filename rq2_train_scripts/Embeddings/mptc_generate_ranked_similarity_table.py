...
def generate_model_tables(df, model, output_dir="mptc_latex"):
...
def generate_visualizations(df, output_dir="mptc_latex"):
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == "__main__":
    os.makedirs("mptc_latex", exist_ok=True)
    df = compile_ranked_similarity()
    df.to_csv("mptc_latex/compiled_similarity.csv", index=False)

    for model in ["xlmr", "mbert"]:
        generate_model_tables(df, model)

    generate_visualizations(df)
    print("[SUCCESS] Clean LaTeX tables and PNG charts saved to: mptc_latex/")
