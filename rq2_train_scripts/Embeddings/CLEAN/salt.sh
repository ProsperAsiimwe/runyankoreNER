...
PY="rq2_train_scripts/Embeddings/CLEAN/salt.py"
  ...
  for i in "${!configs[@]}"; do
    ...
    for CONTEXT_WINDOW in "${CONTEXT_WINDOWS[@]}"; do
      ...
      for MAX_TOKENS_PER_TYPE in "${TOKEN_LIMITS[@]}"; do
        ...
        OUTDIR="rq2_train_scripts/Embeddings/CLEAN/outputs/salt/${MODEL}/config_${i}_cls${USE_CLS}_ctx${USE_CONTEXT}/tokens_${MAX_TOKENS_PER_TYPE}_ctx${CONTEXT_WINDOW}"
        mkdir -p "$OUTDIR"
