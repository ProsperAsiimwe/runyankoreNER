#!/usr/bin/env bash
# run_mptc_best.sh
# Convenience wrapper for: mptc_select_best_and_plot.py
# Usage examples:
#   ./run_mptc_best.sh
#   ./run_mptc_best.sh --models xlmr --techniques core,per_entity
#   ROOT_OVERRIDE=/abs/path/to/outputs/mptc ./run_mptc_best.sh --outdir summary_best_xlmr

set -euo pipefail

# Resolve paths
SCRIPT_DIR="$(cd -- "$(dirname "${BASH_SOURCE[0]}")" &>/dev/null && pwd)"
PY_SCRIPT="$SCRIPT_DIR/mptc_select_best_and_plot.py"

if [[ ! -f "$PY_SCRIPT" ]]; then
  echo "Error: Can't find mptc_select_best_and_plot.py next to this script." >&2
...
DEFAULT_ROOT="${ROOT_OVERRIDE:-outputs/mptc}"
