...
def parse_model_from_path(path_parts: List[str]) -> Optional[str]:
    # look for ".../outputs/mptc_extended/<model>/..."
    for i in range(len(path_parts) - 1):
        if path_parts[i].lower() == "mptc_extended" and i + 1 < len(path_parts):
...
def main():
    ...
    ap.add_argument("--base-dir", type=str, default="rq2_train_scripts/Embeddings/EXTENDED/outputs/mptc_extended",
