...
def generate_latex_table(embedding_name, scores, languages, output_filename):
    ...
    table_lines.append("\\begin{tabular}{l*" + str(len(sample_sizes) * len(datasets)) + "{c}}")
    table_lines.append("\\toprule")
    table_lines.append("\\textbf{Lang} & " + " & ".join(
        [f"\\multicolumn{{3}}{{c}}{{\\textbf{{{size} Sent.}}}}" for size in sample_sizes]) + " \\\\")
    table_lines.append("".join(["\\cmidrule(lr){" + f"{i*3+2}-{i*3+4}" + "}" for i in range(len(sample_sizes))]))
...
for emb_label, emb_filename in embedding_variants.items():
    ...
    for sample in sample_sizes:
        for dataset in datasets:
            file_path = os.path.join(csv_base_dir, dataset, "outputs_sample_size_tuning", str(sample), emb_filename)
    ...
    output_tex_file = f"full_text_cosine_similarity_table_{emb_label.lower().replace('-', '')}.tex"
