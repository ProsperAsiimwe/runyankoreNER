  ...
  for i in "${!configs[@]}"; do
    ...
    for CONTEXT_WINDOW in "${CONTEXT_WINDOWS[@]}"; do
      for MAX_TOKENS_PER_TYPE in "${TOKEN_LIMITS[@]}"; do
        ...
        OUTDIR="rq2_train_scripts/Embeddings/MEXA_inspired_strategy/outputs/combined/${MODEL}/config_${i}_cls${USE_CLS}_ctx${USE_CONTEXT}_hyb${USE_HYBRID}/tokens_${MAX_TOKENS_PER_TYPE}_ctx${CONTEXT_WINDOW}"
        mkdir -p "$OUTDIR"
        ...
        CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES python3 rq2_train_scripts/Embeddings/MEXA_inspired_strategy/combined.py \
          --model_type "$MODEL" \
