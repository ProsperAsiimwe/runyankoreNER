...
def extract_config_info(path):
    # Example: combined/xlmr/config_5_clsfalse_ctxfalse_hybtrue/tokens_300_ctx2/
...
def generate_latex_landscape_table(df, model):
    ...
    output_dir = os.path.join("combined_latex_detailed", model, "landscape_tables")
    os.makedirs(output_dir, exist_ok=True)

    config_groups = model_df.groupby(["CLS", "CTX", "HYB"])
    for (cls, ctx, hyb), group in config_groups:
        rows = []
        caption = f"Similarity results for {model.upper()} with CLS={cls}, CTX={ctx}, HYB={hyb} on the COMBINED dataset."
        label = f"tab:combined_{model}_cls{cls}_ctx{ctx}_hyb{hyb}"
