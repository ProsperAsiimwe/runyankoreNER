...
typological_aux_langs = ["ibo", "xho", "bam"]
masakha_dir = os.path.join(base_dir, "MasakhaNER2.0")
output_dir = os.path.join(base_dir, "TypologicalGroups/G5")
...
for runyankore in runyankore_sources:
    ...
    out_folder = os.path.join(output_dir, f"G5_{runyankore}")
    ...
    print(f"[✓] Created combined train.txt for G5_{runyankore}")
    ...
    print(f"[✓] Copied dev.txt and test.txt for G5_{runyankore}")
