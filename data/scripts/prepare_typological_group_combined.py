...
typological_aux_langs = ["bam", "bbj", "ewe", "fon", "hau", "ibo", "mos", "pcm", "twi", "wol", "yor"]
masakha_dir = os.path.join(base_dir, "MasakhaNER2.0")
output_dir = os.path.join(base_dir, "TypologicalGroupCombined_Unrelated_Languages")
...
for runyankore in runyankore_sources:
    ...
    out_folder = os.path.join(output_dir, f"TypoGroup_{runyankore}")
    ...
    print(f"[✓] Created combined train.txt for TypoGroup_{runyankore}")
    ...
    print(f"[✓] Copied dev.txt and test.txt for TypoGroup_{runyankore}")
